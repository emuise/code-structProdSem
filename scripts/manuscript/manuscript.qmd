...
bec_tab %>%
  ...
  tab_header(title = "Biogeoclimatic Ecosystem Classification (BEC) Zones, their aggregated biomes, and their average climate values for precipitation, maximum temperature, and minimum temperature. Climate data from Wang et al. 2016 averaged for 1991-2020.")
```
...
```{r}
#| label: fig-rda-var-template
#| fig-cap: Template for visualization of RDA and variation partitioning results. Paths between RDA axes and predictor/response variables will be labelled with axis loadings, caluclated as the correlation between the variable and the RDA axis. If loadings are non-significant, NS will be shown on the path.


knitr::include_graphics(here::here("outputs", "template.drawio.png"))
```
...
knitr::include_graphics(here::here("outputs", "drawio", "British Columbia.drawio.png"))
```
