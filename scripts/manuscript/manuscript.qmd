...
```{r}
#| label: fig-rda-var
#| fig-cap: A) Axis loadings from redundancy analysis of primary and modelled forest structure variables on the dynamic habitat indices. B) Results from variation partitioning of primary and modelled forest structure variables on the DHIs. Both visualized analyses are across all collected samples. See supplementary information for results from each BEC zone and forest type.
...
#| fig-cap: Stacked bar plot proportion of DHI variation explained by extracted, modelled, and the overlap between primary and modelled structure variables.
...
knitr::include_graphics(here::here("outputs", "rgb_norm_edit.png"))
```
